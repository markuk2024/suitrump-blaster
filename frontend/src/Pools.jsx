...
function Pools({ walletAddress, onSelectPool, onBack }) {
  ...
  const handleJoinPool = async (pool) => {
    ...
    try {
      ...
      // Parse entry fee - handle both numeric strings and "X.X SUI" format
      const entryFeeNumeric = parseFloat(entryFee.replace(/[^\d.]/g, ''));
      const entryFeeMist = entryFeeNumeric * 1_000_000_000;
      ...
      // For shared objects, use txb.object with the object ID
      txb.moveCall({
        target: `${packageId}::pool::deposit_and_join`,
        arguments: [
          { kind: "object", value: poolObjectId },
          feeCoin,
